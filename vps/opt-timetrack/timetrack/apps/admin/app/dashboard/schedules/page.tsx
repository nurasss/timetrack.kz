...
import { z } from 'zod';
import { WorkSchedule, DayOfWeek } from '@/lib/types';
import { apiGetSchedules, apiCreateSchedule, apiUpdateSchedule, apiDeleteSchedule, apiGetEmployees } from '@/lib/api/service';
import { PageLoader } from '@/components/shared';
import { toast } from 'sonner';

const DAYS: { key: DayOfWeek; label: string; full: string }[] = [
  { key: 0, label: 'Пн', full: 'Понедельник' },
  { key: 1, label: 'Вт', full: 'Вторник' },
  { key: 2, label: 'Ср', full: 'Среда' },
  { key: 3, label: 'Чт', full: 'Четверг' },
  { key: 4, label: 'Пт', full: 'Пятница' },
  { key: 5, label: 'Сб', full: 'Суббота' },
  { key: 6, label: 'Вс', full: 'Воскресенье' },
];
...
export default function SchedulesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      emps.data.forEach(e => {
        if (e.workScheduleId) counts[e.workScheduleId] = (counts[e.workScheduleId] ?? 0) + 1;
      });
      setEmployeeCounts(counts);
    } catch {
      toast.error('Не удалось загрузить графики');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSave(data: ScheduleFormData) {
    setSaving(true);
    try {
      const payload = { ...data, daysOfWeek: data.daysOfWeek as DayOfWeek[], breakDurationMinutes: 0 };
      if (editTarget) {
        const updated = await apiUpdateSchedule(editTarget.id, payload);
