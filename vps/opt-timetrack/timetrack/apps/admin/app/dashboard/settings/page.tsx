...
import {
  ...
  ChevronRight, Check, AlertCircle, Copy, Eye, EyeOff,
  Lock, Users
} from 'lucide-react';
import { toast } from 'sonner';
import { apiGetCompany, apiUpdateCompany } from '@/lib/api/service';
...
const INTEGRATIONS = [
  {
    ...
    desc: 'Выгрузка табеля в формате 1С для расчёта зарплаты.',
    status: 'soon',
    icon: '1С',
  },
  {
    key: 'bitrix',
    label: 'Bitrix24',
    desc: 'Синхронизация сотрудников и уведомления в Bitrix24.',
    status: 'soon',
    icon: 'B24',
  },
  {
    key: 'hikvision',
    label: 'Hikvision',
    desc: 'Интеграция с турникетами и IP-камерами Hikvision.',
    status: 'soon',
    icon: 'HIK',
  },
  {
    key: 'telegram',
    label: 'Telegram Bot',
    desc: 'Уведомления об опозданиях и заявках в Telegram.',
    status: 'soon',
    icon: 'TG',
  },
  {
    key: 'webhook',
    label: 'Webhook',
    desc: 'Отправка событий на ваш URL при каждой отметке.',
    status: 'soon',
    icon: '{}',
  },
];
...
function CompanySettings() {
  ...
  useEffect(() => {
    apiGetCompany().then(c => {
      setForm({ name: c.name, bin: c.bin ?? '', timezone: c.timezone ?? 'Asia/Almaty' });
      setLoading(false);
  ...
  async function handleSave() {
    try {
      await apiUpdateCompany({ name: form.name, bin: form.bin || undefined, timezone: form.timezone });
      setSaved(true);
  ...
  if (loading) return <div className="text-sm text-slate-400 py-4">Загрузка...</div>;
    ...
    <div className="space-y-4">
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
        <p className="text-sm text-slate-600">
          Интеграции будут доступны в следующих версиях Timetrack.kz. Подпишитесь на обновления, чтобы узнать первыми.
        </p>
      </div>
      <div className="grid grid-cols-1 gap-3">
        {INTEGRATIONS.map(integration => (
          <div key={integration.key} className="card p-4 flex items-center justify-between opacity-75">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-sm font-bold text-slate-600">
                {integration.icon}
              </div>
              <div>
                <p className="font-medium text-slate-800">{integration.label}</p>
