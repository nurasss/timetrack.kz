...
function verifyOwnerPin(body, slug, purpose = 'pin') {
  ...
  if (Number.isFinite(lockUntil) && lockUntil > now) return { ok: false, status: 429, error: 'Слишком много попыток. Повторите позже.' };
...
function normalizeStoredState(state) {
  const employees = (Array.isArray(state?.employees) ? state.employees : []).map((value) => employee(value || {})).filter((value) => value.id && value.fname && value.lname);
  ...
  const logs = (Array.isArray(state?.logs) ? state.logs : []).map((value) => {
    ...
    return {
      id: logId, employeeId, empId: employeeId,
      empName: employeeRecord ? `${employeeRecord.fname} ${employeeRecord.lname}`.trim() : text(input.empName, 180),
      empIin: employeeRecord?.iin || '', empPhoto: employeeRecord?.photo || null, type,
      time: kazakhstanTimeString(when), date: kazakhstanDisplayDate(when), ts: when.toISOString(), at: when.toISOString(), eventTimestamp,
      isLate: type === 'checkin' ? employeeLate(employeeRecord, state.settings, when) : false,
      verification: manual ? 'manual' : '', manual,
  ...
  return { employees, logs, settings: normalizedSettings };
}
...
function buildAttendanceLog(input, employeeRecord, state, { manual = false, serverNow = new Date() } = {}) {
  ...
  const previous = state.logs.filter((item) => item.empId === input.employeeId).sort((a, b) => attendanceEventMs(a) - attendanceEventMs(b) || timestamp(a.ts) - timestamp(b.ts));
  ...
  return { ok: true, log: {
    id: input.id, employeeId: employeeRecord.id, empId: employeeRecord.id,
    empName: `${employeeRecord.fname} ${employeeRecord.lname}`.trim(), empIin: employeeRecord.iin || '', empPhoto: employeeRecord.photo || null,
    type: expectedType, time: kazakhstanTimeString(eventDate), date: kazakhstanDisplayDate(eventDate), ts: serverNow.toISOString(), at: serverNow.toISOString(), eventTimestamp: eventDate.toISOString(),
    isLate: expectedType === 'checkin' ? employeeLate(employeeRecord, state.settings, eventDate) : false,
    verification: manual ? 'manual' : '', manual, manualPhoto: '',
...
async function handleAPI(req, res) {
  ...
  const requestLimits = {
    register: [5, 3600000], checkAdminPin: [30, RATE_LIMIT_WINDOW_MS], requestPinReset: [10, RATE_LIMIT_WINDOW_MS], resetPin: [10, RATE_LIMIT_WINDOW_MS], confirmEmail: [20, RATE_LIMIT_WINDOW_MS], resendConfirmation: [10, RATE_LIMIT_WINDOW_MS], verifyTabletAccess: [20, RATE_LIMIT_WINDOW_MS], addLog: [120, RATE_LIMIT_WINDOW_MS], addManualLog: [30, RATE_LIMIT_WINDOW_MS], changeAdminPin: [10, RATE_LIMIT_WINDOW_MS],
  };
  ...
  if (action === 'checkAdminPin') {
    ...
    let auth;
    if (matchedUser) {
      if (userLoginLocked(security, login)) return json(res, 429, { ok: false, error: 'Слишком много попыток входа', code: 'LOGIN_RATE_LIMITED' });
    ...
    if (pinChangeRequired) return json(res, 200, { ok: true, pinChangeRequired: true, csrfToken: session.csrfToken, role, userName: sessionUser.name || 'Администратор' });
  ...
  if (action === 'changeAdminPin') {
    ...
    if (!auth.ok) return json(res, auth.status, { ok: false, error: auth.error, code: auth.code });
    if (!allowRole(auth, ['owner', 'admin'])) return json(res, 403, { ok: false, error: 'Недостаточно прав для смены PIN', code: 'FORBIDDEN' });
    ...
    const newPin = String(body.newPin || '').trim();
    if (!validNewAdminPin(newPin)) return json(res, 422, { ok: false, error: 'PIN должен содержать 6–12 цифр и не быть слишком простым' });
  ...
  if (action === 'addManualLog') {
    ...
    const built = buildAttendanceLog(parsed.value, employeeRecord, state, { manual: true, serverNow: new Date() });
    if (!built.ok) return json(res, built.status, { ok: false, error: built.error, code: built.code, state: tabletState(state) });
  ...
  if (action === 'updateSettings') {
    if (body.settings?.adminPin || body.settings?.adminPinHash) return json(res, 422, { ok: false, error: 'Смена PIN требует currentPin', code: 'CURRENT_PIN_REQUIRED' });
