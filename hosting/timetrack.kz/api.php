...
function read_state_split(string $companyDir): array
{
    ...
    if (is_file($employeesPath) || is_file($settingsPath) || is_file($logsPath)) {
        ...
        if (($employeesRaw !== null && !isset($employeesRaw['employees']) || $employeesRaw !== null && !is_array($employeesRaw['employees']))
...
function tablet_state(array $state): array
{
    ...
    return [
        ...
        'settings' => [
            ...
            'matchThreshold' => (float)($state['settings']['matchThreshold'] ?? 0.55),
...
try {
    ...
    if ($action === 'saveEmployee') {
        if (!empty($input['employee']['photo']) && normalize_photo($input['employee']['photo'] ?? null) === null) {
            respond(['ok' => false, 'error' => 'Фото должно быть в формате JPEG, PNG или WebP'], 422);
        }
    ...
    if ($action === 'addManualLog') {
        ...
        $state['logs'] = array_slice($state['logs'], 0, 10000);
        write_logs_file($logsFile, $state);
        append_audit($pinAuth['security'], [
            'action' => 'manualAttendance',
            'deviceId' => $device['id'] ?? '',
            'logId' => $built['log']['id'],
            'employeeId' => $built['log']['employeeId'],
            'reason' => $reason,
        ]);
